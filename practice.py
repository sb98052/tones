...
Practice‑session player
  ...
  • **macOS enhancement**: Before the second strike of every chord, the
    built‑in **`say`** TTS engine speaks the **scale‑degree numbers** (1‑7)
    of that voicing and blocks until it finishes, so your ear can identify
    the harmony hands‑free.
"""
...
class Player:
    ...
    # internal helpers ------------------------------------------------
    def _snd(self, n):
        return pygame.mixer.Sound(str(self.f / f"{n}.mp3"))

    def _say_degrees(self, degs: Sequence[int]):
        """Speak 1‑based scale degrees synchronously (macOS only)."""
    ...
    def note(self, n, show, deg):
        ...
        ch.set_volume(self.vol)
        time.sleep(self.dur)
        ch.stop()

    def chord(self, notes, degs, show):
        label = ", ".join(
            f"{n}({d + 1})" for n, d in zip(notes, degs)
        ) if show else ", ".join(f"({d + 1})" for d in degs)
        print("→ CHORD", label)
        chs = [self._snd(n).play(loops=0, maxtime=int(self.dur * 1000)) for n in notes]
        for c in chs:
            c.set_volume(self.vol)
        time.sleep(self.dur)
        for c in chs:
            c.stop()

    # convenience wrapper used by Session to announce before 2nd strike
    def chord_with_announcement(self, notes, degs):
        # first strike (silent degrees)
        self.chord(notes, degs, False)
        # speak scale degrees synchronously, then strike again
        time.sleep(3)
        self._say_degrees(degs)
        # optional short pause so the ear resets before the chord re‑sounds
        time.sleep(0.4)
        self.chord(notes, degs, False)
...
class Gen:
    def __init__(self, key: Key):
        self.key, self.scale = key, key.scale
        self.len = len(self.scale)
    ...
    def triad(self):
        ...
        if inv == 0:
            idx = [root, root + 2, root + 4]  # root
        elif inv == 1:
            idx = [root + 2, root + 4, root + 7]  # 1st
        else:
            idx = [root + 4, root + 7, root + 9]  # 2nd
...
class Session:
    ...
    def _cadence(self):
        ...
        self.cad = []
        for r in roots:
            notes = [self.g.scale[r], self.g.scale[r + 2], self.g.scale[r + 4]]
            degs = [self.g.key.degree(n) for n in notes]
            self.cad.append((notes, degs))

    # helpers ---------------------------------------------------------
    def _pitch(self, n):
        base, octv = n[:-1], int(n[-1])
        return octv * 12 + NOTE_TO_SEMI[base]
    ...
    def run(self):
        try:
            ...
            if self.ton == 'major':
                for i in range(0, 8):
                    n = self.g.scale[i]
                    self.p.note(n, True, i)
            else:
                for i in range(5, 13):
                    n = self.g.scale[i]
                    self.p.note(n, True, i - 5)
            print("\n▶ Free play:")
            kinds = ['chord']  # we only need chords for ear training
            ...
            while time.time() - self.start < SESSION_DUR:
                ...
                # reverse only for non‑chord types (not used here)
                ...
                if self.count % self.CAD_FREQ == 0:
                    for n, d in self.cad:
                        self.p.chord(n, d, False)

                # ---------- dispatch ----------
                if kind == 'chord':
                    self.p.chord_with_announcement(notes, degs)
                else:
                    # other types are kept for completeness
                    for n, d in zip(notes, degs):
                        self.p.note(n, False, d)
